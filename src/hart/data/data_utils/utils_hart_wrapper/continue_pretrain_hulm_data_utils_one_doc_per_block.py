...
def concat(data, user_id_column, text_column, text_id_column):
    # return data.groupby(user_id_column).agg({'batch_encodings': 'sum', text_column: lambda x: ' '.join(x)}).reset_index()
    return data.groupby(user_id_column).agg({'batch_encodings': 'sum', text_column: ' '.join, text_id_column: list}).reset_index()
...
def process_data(data, tokenizer, text_column, block_size):
    ...
    def process(data):
        ...
        tokenized['labels'] = pad_or_truncate(tokenized['labels'], -100, tokenizer.sep_token_id)
        return [BatchEncoding(dict(input_ids = tokenized['input_ids'], 
                            attention_mask=tokenized['attention_mask'], labels = tokenized['labels']))]
