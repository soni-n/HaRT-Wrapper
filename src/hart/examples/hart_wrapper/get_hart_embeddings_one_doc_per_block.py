...
from ...data.data_utils.utils_hart_wrapper.inference_data_utils_one_doc_per_block import load_tokenized_dataset as load_dataset
...
def get_hart_embeddings(
    ...
    if return_all_user_states:
        ...
        all_text_ids_with_user_ids_df = pd.DataFrame.from_dict(
            all_text_ids_with_user_ids, orient='index').rename(columns={0: 'text_ids'})
        all_user_states_with_user_ids_df = all_user_states_with_user_ids_df.merge(all_text_ids_with_user_ids_df, left_index=True, right_index=True)
