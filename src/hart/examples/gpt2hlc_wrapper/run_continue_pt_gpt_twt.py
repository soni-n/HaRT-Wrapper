...
from ...model_gpt2hlc.gpt2hlcLMhead import GPT2hlcLMHeadModel
# from  ...model_gpt2hlc.gpt2hlcLMhead import GPT2hlcLMHeadModel

from ...args.clm_args import DataTrainingArguments, ModelArguments
from ...data.data_utils.utils_gpt2hlc_wrapper.continue_pretrain_data_utils import load_dataset
...
def main():
    ...
    if data_args.train_table is not None or data_args.dev_table is not None or data_args.test_table is not None:
        if data_args.train_table is not None:
            train_data, train_uncut_blocks = load_dataset(logger, tokenizer, data_args.train_table, block_size, data_args.max_train_blocks, text_column=data_args.text_column, user_id_column=data_args.user_id_column)
        if data_args.dev_table is not None:
            eval_data, eval_uncut_blocks = load_dataset(logger, tokenizer, data_args.dev_table, block_size, data_args.max_val_blocks, text_column=data_args.text_column, user_id_column=data_args.user_id_column)
        elif data_args.test_table is not None:
            eval_data, eval_uncut_blocks = load_dataset(logger, tokenizer, data_args.test_table, block_size, data_args.max_val_blocks, text_column=data_args.text_column, user_id_column=data_args.user_id_column)
    else:
